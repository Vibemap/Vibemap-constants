...
var badges = [
	{
		...
		map: {
			address: "Civic Center, San Francisco, CA, USA",
			lat: 37.7815533,
			lng: -122.4156427,
			city: "San Francisco",
			name: "Civic Center",
		...
		icon: {
			id: 50170,
			url: "https://cms.vibemap.com/wp-content/uploads/2022/03/Van-Ness-Bus-Rapid-Transit.jpg",
			icon: "https://cms.vibemap.com/wp-includes/images/media/default.png"
		},
		name: "Van Ness Bus Badge"
	},
	{
		id: 49930,
		slug: "49930",
		...
		key: "49930",
		description: "<p>We&#8217;re excited to announce a new partnership with Oakland&#8217;s OACC and Cut Fruit Collectivelaunching April 16th. Stay tuned for more. : )</p>\n",
		has_location: false,
		location: {
			ID: 1447,
			post_author: "1",
			...
			to_ping: "",
			pinged: "",
			post_modified: "2022-02-15 09:32:00",
			post_modified_gmt: "2022-02-15 17:32:00",
		...
		icon: {
			id: 49911,
			url: "https://cms.vibemap.com/wp-content/uploads/2022/03/209914268_1011119466365184_5484193727252803894_n-e1646863593806.jpg",
			icon: "https://cms.vibemap.com/wp-includes/images/media/default.png"
		},
		name: "(Coming Soon!) OACC & Cut Fruit Collective Badge"
	},
	{
		id: 43587,
		slug: "first-fridays",
...
const getCategoryMatch = (categories = ['all']) => {
...
const graphToEvents = (edges = []) => {
  const events = edges.map(edge => {
    ...
    const nextStartTime = dayjs__default["default"](recurRule.next(1).toLocaleString()
      .replace(`00:00:00`, startTime));

    const nextEndTime = dayjs__default["default"](recurRule.next(1).toLocaleString()
...
const fetchEvents = async (options, activitySearch = false) => {
  ...
  let response = await Axios__default["default"].get(`${apiEndpoint}?${query}`, {
...
const nextDateFromRecurring = (...[
  ...
  const date = dayjs__default["default"](); // .startOf('month')
 ...
 * @returns {Object[]}
 */
const sortNeighborhoodsByVibes = (neighborhoods, vibeSlugs) => {
  if (vibeSlugs.length === 0) return neighborhoods

  const relatedVibeSlugs = vibes.getRelatedVibes(vibeSlugs);
...
exports.sortNeighborhoodsByVibes = sortNeighborhoodsByVibes;
